...
relax2es = fittype(@(q0,q1,p1,t)...
    eps0*(q0*(1-exp(-1/p1*(t-data.time(s0_loc))))+...
    q1/p1*exp(-1/p1*(t-data.time(s0_loc)))),'independent','t');

% Relax 3ES Analytical Solution
relax3es=fittype(@(q0,q1,p1,t)...
    b*(q0*t-q1*(exp(-t/p1)-1)+p1*q0*(exp(-t/p1)-1))-b*(q0*(t-data.time(s0_loc))-...
    q1*(exp(-(t - data.time(s0_loc))/p1) - 1) +...
    p1*q0*(exp(-(t - data.time(s0_loc))/p1) - 1)),'independent','t');
